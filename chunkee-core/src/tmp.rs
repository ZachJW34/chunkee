...
use chunkee_core::{
    chunk::Chunk32,
    coords::{cv_to_wv, wv_to_cv, ChunkVector, LocalVector, WorldVector},
...
impl WorldGenerator {
    ...
    fn get_or_compute_column_data(&self, cv: ChunkVector) -> Arc<ChunkColumnData> {
        ...
        for x in 0..32 {
            for z in 0..32 {
                let world_x = ((chunk_world_pos.x + x) as f32 * self.voxel_size) as f64;
                let world_z = ((chunk_world_pos.z + z) as f32 * self.voxel_size) as f64;
        ...
        for x in 0..side {
            for z in 0..side {
                ...
                for y in 0..side {
                    ...
                    let voxel_world_pos = (chunk_start + lv).as_vec3() * self.voxel_size;
                    ...
                    if voxel_world_pos.y > SEA_LEVEL {
                        // Is this voxel the surface?
                        if voxel_world_pos.y <= terrain_height {
                    ...
                    // Place underground blocks (only if not air or water)
                    if block_to_place == MyVoxels::Air && voxel_world_pos.y < terrain_height {
                        if voxel_world_pos.y > terrain_height - (3.0 * self.voxel_size) {
